...
YOLOが登場する前は、**R-CNN**（領域ベースのCNN）というモデルが主流でした。R-CNNは「ここに物体がありそう」という候補をいくつも探し出し、その一つ一つに対して「それは何か」を判定する**2段階**（Two-Stage）のアプローチをとっていました。精度は高いものの、処理に時間がかかる点が弱点でした。
...
2.  **マス目ごとに並列で予測する:** 各グリッドが、「自分の中心に物体があるか？」「それは何か？」「枠の大きさは？」という情報を**すべて同時に計算**します。
...
この驚異的なスピードの根本を長年支えてきたのが、「**Darknet（ダークネット）**」と呼ばれるシステムです。これはYOLOの生みの親であるJoseph Redmon氏が独自にC言語から作り上げたディープラーニング・フレームワークです。

**Darknetの技術的な特徴**

- 当時の巨大になりがちなフレームワーク（TensorFlowなど）に対し、「誰でも読めるシンプルなソースコードで、とにかく計算を身軽に速くする」というハッカー的な思想を持って設計されました。
- 初期のYOLOに積まれていた「Darknet-19」や「Darknet-53」といったモデル（バックボーン）は、複雑な分岐を持たず、小さなフィルター（$3 \times 3$ と $1 \times 1$）を素直に重ねていく**「無駄のない一直線の畳み込み構造」**をとっています。
- これにより、非常に軽量でありながら、CPU上でもそこそこ動き、GPUを使えば爆速で推論できるシステムが実現しました。

#### 3. PyTorchへの移行による完全なる普及
...
YOLO（`ultralytics`ライブラリ）を動かすための前提となる環境は以下の通りです。
