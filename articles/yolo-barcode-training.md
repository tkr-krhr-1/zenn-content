...
Google Colabにセルを追加してRoboflowのコードを実行します。
...
学習を始める前に、あらかじめW&Bへのアカウント登録とAPIキーの取得が必要です。[Weights & Biasesの公式サイト](https://wandb.ai/)にアクセスしてアカウントを作成(またはログイン)し、アカウント設定ページからAPIキーをコピーしておきましょう。

APIキーが準備できたら、セルを追加し以下のコードを実行します。
これにより、ライブラリのインストールとW&Bへのログインが行われます。
APIキーの入力画面が表示されるので、コピーしたAPIキーを貼り付けてEnterキーを押します。
...
ログインが完了したら、セルを追加して以下のコードをコピーし、学習を実行します。
...
:::message
このログが表示されていれば、正常に選択したGPUが使用されています。
...
![学習の進捗ログ](/images/yolo-barcode-training/image7.png)
...
![W&Bのシステムダッシュボード](/images/yolo-barcode-training/image6.png)

### 学習済みモデルの検証

ここからは、学習したモデルを**ローカルPC**にダウンロードし、手元のカメラを使って検証を行う手順を解説します。
実行環境が Colab から ローカルPC へと変わることに注意してください。

#### Google Colabでの作業: モデルのダウンロード

学習が完了すると、Google Colab上の `runs/detect/train/weights/best.pt` にモデルファイルが保存されます。
以下のコードをGoogle Colab上で実行して、モデルをローカルPCにダウンロードします。
...
#### ローカルPCでの作業: 推論スクリプトの実行

ダウンロードした `best.pt` を、ローカルPCの任意の作業ディレクトリに移動させます。
次に、同じディレクトリ内に物体検出を実行するためのPythonスクリプトファイルを以下のコードで作成します。
